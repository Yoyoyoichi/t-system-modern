...
for($i = 0; $i < count($row); $i++){
  ...
  // $question[$i] = str_replace(array("\r\n", "\r", "\n"), '<br>', $row[$i][1]);htmlコードをだすと改行コードが文字列<br>になってしまう
  $question[$i] = $row[$i][1];
  // $question[$i] = $row[$i][1];
  $answer[$i] = $row[$i][0];
  ...
  if (isset($qsentence[$i-1])===false){//値が入っていないかどうか
    // echo "\n"."\n"."x"."\n"."\n";
    $qsentence1 = $qsentence[$i];
    $qsentence2 = $qsentence[$i]."x"; //文字列をずらす
  }else{
    // echo "\n"."\n"."y"."\n"."\n";
    $qsentence1 = $qsentence[$i];
  ...
  if (strpos($answer[$i], '<p ')=== false){//解答がhtml要素かどうか
