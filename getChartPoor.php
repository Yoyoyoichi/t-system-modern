...
for($i = 1; $i < $row_cnt; $i++){
  ...
  for($j = 0; $j < 8; $j++){
    // $time_start = microtime(true);
  	$str_sql = "SELECT * FROM  $db_name where (category1 = '$pieces[$i]') And (poorat = '$poor[$j]')";
  ...
  $str_sql = "SELECT * FROM  $db_name where (category1 = '$pieces[$i]') And ((poorat IS NULL) OR (poorat = ''))";
  // echo "str_sql is ".$str_sql."\n"."\n";
  $result = $mysqli->query($str_sql);
  $row_cnt2 = mysqli_num_rows($result);
  // echo "levels row_cnt is ".$row_cnt2."\n"."\n";
