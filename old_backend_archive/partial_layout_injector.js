...
// Regex to replace the old partial dashboard JS and CSS
...
if (content.match(jsRegex)) {
    content = content.replace(jsRegex, partialDashboardJS);
} else {
    content = content.replace('</body>', partialDashboardJS + '\n</body>');
}

const cssRegex = /\/\* Modern Top Panel CSS \*\/[\s\S]*?(?=<\/style>)/;
if (content.match(cssRegex)) {
    content = content.replace(cssRegex, partialDashboardCSS + '\n');
} else {
    content = content.replace('</style>', partialDashboardCSS + '\n</style>');
}

fs.writeFileSync(file, content);
console.log("Updated partial layout injector added.");
