...
<body>
  <form name ="mainform" action="" method="post">
    ...
    <select name='autoReading' id='autoReading' class="textlines" style='width: 23%; font-size: 20px;'>
        <option value='no'' style='width: 23%' placeholder="隱ｭ荳企浹螢ｰ">隱ｭ荳企浹螢ｰ</option>
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!empty($_POST["DB_name"])) {
    ...
    $str_sql = "SELECT max(recordnumber) FROM  A01tsystemrecord01";//蟋九∪繧翫・譎ょ綾繧貞叙蠕・    // echo $str_sql."\n"."\n";
...
function clickOKNG(OKNG){
  ...
  if((OKNG.value === "Tomorrow")||(OKNG.value === "3 Days")||(OKNG.value === "A Week")||(OKNG.value === "2 Weeks")){
    ...
    var xmlhttp=createXmlHttpRequest();
    if(xmlhttp!=null){
      xmlhttp.open("POST", "../FCaddcorrect20210203.php", false);//荳肴ｭ｣隗｣繝懊ち繝ｳ繧呈款縺・      xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      var data="data="+moji;
      xmlhttp.send(data);
      var res=xmlhttp.responseText;
    }
...
// select繧ｿ繧ｰ縺ｮ荳ｭ霄ｫ繧貞｣ｰ縺ｮ蜷榊燕縺悟・縺｣縺殪ption繧ｿ繧ｰ縺ｧ蝓九ａ繧・function appendVoices() {
  // 竭縲菴ｿ縺医ｋ螢ｰ縺ｮ驟榊・繧貞叙蠕・  // 驟榊・縺ｮ荳ｭ霄ｫ縺ｯ SpeechSynthesisVoice 繧ｪ繝悶ず繧ｧ繧ｯ繝・  const voices = speechSynthesis.getVoices()
  voices.forEach(voice => { //縲繧｢繝ｭ繝ｼ髢｢謨ｰ (ES6)
    // 譌･譛ｬ隱槭→闍ｱ隱樔ｻ･螟悶・螢ｰ縺ｯ驕ｸ謚櫁い縺ｫ霑ｽ蜉縺励↑縺・・    if(!voice.lang.match('ja|en-US')) return
...
// // 竭｡ 菴ｿ縺医ｋ螢ｰ縺瑚ｿｽ蜉縺輔ｌ縺溘→縺阪↓逹轣ｫ縺吶ｋ繧､繝吶Φ繝医ワ繝ｳ繝峨Λ縲・// // Chrome 縺ｯ髱槫酔譛溘↓(荳蛟九★縺､)螢ｰ繧定ｪｭ縺ｿ霎ｼ繧縺溘ａ蠢・ｦ√・speechSynthesis.onvoiceschanged = e => {
  appendVoices()
}
</script>
</body>
