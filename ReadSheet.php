...
value="ひらがな"  style=" width:15vw;height:5vh; font-size: 15px"> -->
<br><br>
...
for (let i = 0; i < scales.length; i++) {//Mp3開始地点セレクト要素追加
    // selectタグを取得する
        ...
        switch(result[0][2][i]){
          case "r":
            if ((onChordRoot===0)||(onChordRoot)){
              fretElem[j].innerHTML = "<span class='noteCircle'  style='color:#F00;'>●</span>";
            // fretElem[j].style.background = 'skyblue';
            }else{
              fretElem[j].innerHTML = "<span class='noteCircle'  style='color:#000;font-size:  40px '>●</span>";
          ...
          case "c":
            ...
            break;
          case "nt":
            fretElem[j].innerHTML = "<span class='noteCircle'  style='color:skyblue;'>●</span>";
            // fretElem[j].style.background = 'palegreen';
            break;
          case "at":
            fretElem[j].innerHTML = "<span class='noteCircle'  style='color:green;'>●</span>";
            // fretElem[j].style.background = 'palegreen';
            break;
          case "bn":
            fretElem[j].innerHTML = "<span class='noteCircle'  style='color:blue;'>●</span>";
            // fretElem[j].style.background = 'palegreen';
            break;
        }
...
function downloadScale(){
  ...
  if(xmlhttp!=null)
  {
    xmlhttp.open("POST", "../getPianoHtml.php", false);//不正解ボタンを押す
...
// var grandParentContanerElem = document.createElement("div"); // p要素作成
...
function LineupScale(){
  ...
  for(i=0;i<chords.length;i++){
    if(chords[i].length===0){
      containerChangeFlag = true;//グランドペアレンツコンテナ切り替えフラグ
    }
    for(j=0;j<chords[i].length;j++){
      if (chords2[i][j].indexOf( '[')<0){continue;}
      let chord = chords[i][j].match(/\[(.+)\]/)[1];//[]の中身を取得
      let chordRoot;
      let chordType;
      if (chord.length===1){
        chordRoot = chord
      } else if ((chord.length===2)&&((chord.substr(1,1)==='#')||(chord.substr(1,1)==='b')||(chord.substr(1,1)==='♭')||(chord.substr(1,1)==='＃'))){
        chordRoot = chord
      } else if ((chord.substr(1,1)==='#')||(chord.substr(1,1)==='b')||(chord.substr(1,1)==='♭')||(chord.substr(1,1)==='＃')){
...
function songNameChange(){
  ...
  let txt = document.getElementById("progressionList").options[idx].value;//textを選ぶと改行が反映されないのでvalueを選ぶ。
...
function addSong(){
  ...
  if(xmlhttp!=null)
  {
    xmlhttp.open("POST", "../addSongProgression.php", false);//不正解ボタンを押す
    xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    var data="data="+moji;
    xmlhttp.send(data);
    var res=xmlhttp.responseText;
...
function deleteSong(){
  ...
  if (check){
    ...
    if(xmlhttp!=null)
    {
      xmlhttp.open("POST", "../deleteProgression.php", false);//不正解ボタンを押す
      xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      var data="data="+moji;
      xmlhttp.send(data);
      var res=xmlhttp.responseText;
...
function addHiragana(){
  ...
  if (hiraganaFrag){
    ...
    for(i=0;i<ChordOrScaleNames.length;i++){
      var chord2 = ChordOrScaleNames[i].innerText.match(/\[(.+)\]/)[1];//[]の中身を取得
      if (chord2.length===1){
        chord2Root = chord2
      } else if (chord2.indexOf('/')>-1){     
        let index = chord2.indexOf('/');
        chord2Root = chord2.substring(index+1);
      } else if ((chord2.length===2)&&((chord2.substr(1,1)==='#')||(chord2.substr(1,1)==='b')||(chord2.substr(1,1)==='♭')||(chord2.substr(1,1)==='＃'))){
        chord2Root = chord2
      } else if ((chord2.substr(1,1)==='#')||(chord2.substr(1,1)==='b')||(chord2.substr(1,1)==='♭')||(chord2.substr(1,1)==='＃')){
