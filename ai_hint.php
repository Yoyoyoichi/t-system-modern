...
if ($is_answer_revealed) {
    if ($depth == 1) {
        ...
        // 解答表示後（2回目）：深掘り
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーは一度解説を読みましたが、さらに深い情報を求めています。\n\n【重要】回答は全体で「3〜4行程度」に収めてください。前回とは異なる、より深い語源、ニュアンスの違い、または記憶に定着する具体的なエピソードを端的に提供してください。";
        $user_message = "以下の問題について、もっと深掘りした解説（より詳しい語源やニュアンスなど）をお願いします。\n\n【問題】\n" . $question . "\n\n【正解】\n" . $answer;
    } else {
        // 解答表示後（3回目以降）：さらに深掘り
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーはさらに高度な情報を求めています。\n\n【重要】回答は全体で「3〜4行程度」に収めてください。実際の会話やビジネスでの実用例文、同義語・反意語との使い分けなど、実践的で高度な知識を提供してください。";
        $user_message = "以下の問題について、さらに高度で実践的な知識（例文や類義語の使い分けなど）を教えてください。\n\n【問題】\n" . $question . "\n\n【正解】\n" . $answer;
    }
} else {
    if ($depth == 1) {
        ...
        $user_message = "以下の問題の答えを自力で出したいので、答えは言わずにヒントをください。\n\n【問題】\n" . $question;
    } else {
        // 解答表示前（2回目以降）：さらに大きなヒント
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーは最初のヒントでは答えが分かりませんでした。直接的な答え（正解）は絶対に教えずに、もう少し踏み込んだ「決定的なヒント」を提供してください。\n\n【重要】回答は非常に簡潔に、全体で「3行程度」に収めてください。";
        $user_message = "最初のヒントでは分かりませんでした。答えは言わずに、もう少しだけ決定的なヒントをください。\n\n【問題】\n" . $question;
    }
    if ($answer) {
