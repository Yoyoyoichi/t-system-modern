...
if ($is_answer_revealed) {
    if ($depth == 1) {
        // 解答表示後（1回目）：解説のみ
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーはすでにこの問題の解答を確認しました。\nこの問題のわかりやすい「解説」を行ってください。\n\n【重要】回答は全体で「4行程度」に収めつつ、学習者が楽しく記憶に定着させやすいように「暗記法」「イメージ法」「面白い語呂合わせ（ダジャレ等）」のいずれかを簡潔に盛り込んでください。問題文や解答自体をそのまま繰り返す必要はありません。";
        $user_message = "以下の問題についての解説をお願いします。\n\n【問題】\n" . $question . "\n\n【正解】\n" . $answer;
    } else if ($depth == 2) {
        // 解答表示後（2回目）：深掘り
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーは一度解説を読みましたが、さらに深い情報を求めています。\n\n【重要】回答は全体で「4行程度」に収めてください。前回とは異なる視点で、記憶に残る強烈なイメージ法や、さらにひねりの効いた語呂合わせ、または語源から連想する覚え方を簡潔に提供してください。";
        $user_message = "以下の問題について、もっと深掘りした解説や、別の角度からの暗記法（語呂合わせなど）をお願いします。\n\n【問題】\n" . $question . "\n\n【正解】\n" . $answer;
    } else {
        // 解答表示後（3回目以降）：さらに深掘り
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーはさらに高度な情報を求めています。\n\n【重要】回答は全体で「4行程度」に収めてください。実用例文、同義語・反意語との使い分けなどに加え、それらをひっくるめて覚えるためのユーモアを交えた独自の暗記法やイメージを簡潔に提案してください。";
        $user_message = "以下の問題について、さらに高度で実践的な知識（例文や類義語など）と、それを覚えるための面白いイメージ法や語呂合わせを教えてください。\n\n【問題】\n" . $question . "\n\n【正解】\n" . $answer;
    }
} else {
    if ($depth == 1) {
        ...
        $system_prompt = "あなたは優秀な学習サポートAIです。ユーザーは最初のヒントでは答えが分かりませんでした。直接的な答え（正解）は絶対に教えずに、もう少し踏み込んだ「決定的なヒント」を提供してください。\n\n【重要】回答は全体で「4行程度」に収めてください。答えに直結するような強力なイメージや、答えの一部がわかるような語呂合わせの「前半部分」などを交えて、答えを閃かせるサポートをしてください。";
        $user_message = "最初のヒントでは分かりませんでした。答えは言わずに、もう少しだけ決定的なヒント（強力なイメージや語呂合わせなど）をください。\n\n【問題】\n" . $question;
    }
    if ($answer) {
